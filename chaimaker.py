...
class ChaiMaker:
    ...
    def get_value_of_assignee(self, assignee):
        ...
        if isinstance(assignee, Int):
            ...
            if self.is_updated_after_comp(assignee):
                raise Exception("Line {}: Assignee '{}' value error: Cannot obtain the value because it will "
                                "be updated after compilation".format(assignee.lineno, npid))
            else:
        ...
        if isinstance(element, str) or (isinstance(element, Int) and not self.is_updated_after_comp(element)):
            ...
            if len(expression) == 1:
                ...
                if isinstance(assignee, str) or (isinstance(assignee, Int) and not self.is_updated_after_comp(
                    ...
                    logging.debug("Array '{}' index is: {}".format(array_pid, value))
                    ...
                    logging.debug("Array '{}' index is stored at memory index: {}".format(array_pid, var_index))
                ...
                if (isinstance(a, str) and isinstance(b, str)) \
                    ...
                    var_array_element_index = self.get_mem_index(element)
    ...
    def handle_index_expression_assignment(self, expression, index, pid, is_storing_values):
        ...
        if len(expression) == 1:
            ...
            if (isinstance(a, str) or (isinstance(a, Int) and not self.is_updated_after_comp(a))) \
                ...
                # a_vid, b_vid, a_arr_mem, a_arr_offset, b_arr_mem, b_arr_offset = self.return_condcheck_preparations(a,
                #                                                                                                     b)

                return gen_assign_var_to_expr(assign_to_var_index=index,
                                              a_vid=a_vid,
                                              b_vid=b_vid,
                                              a_const=isinstance(a, str),
                                              b_const=isinstance(b, str),
                                              is_a_arra=isinstance(a, IntArrayElement),
                                              a_array_offset=a_array_offset,
                                              a_array_mem_index=a_array_mem_index,
                                              is_b_array=isinstance(b, IntArrayElement),
                                              b_array_offset=b_array_offset,
                                              b_array_mem_index=b_array_mem_index,
                                              operand=operand,
    ...
    # TODO: Write better method or remove
    ...
    def handle_if_statement(self, statement):
        ...
        if (isinstance(a, str) or (isinstance(a, Int) and not self.is_updated_after_comp(a))) \
            ...
            if comparision_result:
                ...
                return self.delegate_translation(commands)
            else:
                if isinstance(statement, IfElse):
                    # Execute alternative set of statements
                    return self.delegate_translation(alt_commands)
                else:
            ...
            condition_len = len(result)
            self.program_counter += condition_len

            # Update program counter values in translated statement to correct ones
            result.extend(self.delegate_translation(commands))
            ...
            if isinstance(statement, IfElse):
                ...
                result.extend(self.delegate_translation(alt_commands))

                self.program_counter -= 1

            # Revert pc val back to normal
            self.program_counter -= condition_len
    ...
    def handle_do_while_statement(self, statement):
        ...
        condition_len = len(cond_statement)
        self.program_counter += condition_len
    ...
    def handle_for_statement(self, statement):
        ...
        if pid not in self.global_vars.keys():
            ...
            self.iterators[pid] = True
            self.var_index[pid] = self.next_free_mem_index
            self.next_free_mem_index += 1

        elif pid in self.global_vars.keys():

            if pid not in self.iterators.keys():
        ...
        # Revert program counter back to normal
        self.program_counter -= condition_len
    ...
    def handle_for_downto_statement(self, statement):
        ...
        if pid not in self.global_vars.keys():
            ...
            if not pid in self.iterators.keys():
                raise Exception("Iterator pidentifier '{}' already declared".format(pid))
    ...
    def make(self):
        ...
        outf = open('tests/dev_tests/program2.o', 'w').write('\n'.join(output))
